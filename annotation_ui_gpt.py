...
def generate_markdown(transcript_id, petitioner_or_respondent, justice_name):
    ...
    return f"""
        # {brief_json["name"]} (Docket {get_docket(transcript_id)}, {get_year(transcript_id)}) [GPT]
        ### Facts of the Case
        ...
        {"<br>".join([f"• {get_text(x)}'" for x in coherent_questions])}
...
with gr.Blocks(theme=gr.themes.Ocean(), css=css) as demo:
    ...
    with gr.Column() as config_setup_row:
        ...
        question_select_button = gr.Button(value="Annotate!", visible=False)
